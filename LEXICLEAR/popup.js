...
/** Save to history: send to background (reliable when popup closes) and also save locally. */
function saveActivityToHistory(type, input, resultSummary) {
  var urlOrInput = (input != null && input !== '') ? String(input) : '';
  var payload = {
    type: type,
    input: urlOrInput,
  ...
  if (chrome.runtime && chrome.runtime.sendMessage) {
    chrome.runtime.sendMessage({ action: 'SAVE_HISTORY', payload: payload });
  }
  var item = {
  ...
  chrome.storage.local.get(['history'], function (data) {
    var list = Array.isArray(data.history) ? data.history : [];
    var next = [item].concat(list).slice(0, 100);
    chrome.storage.local.set({ history: next }, function () {
...
/** Show URL in history like Chrome: hostname or full URL truncated. */
function getDisplayUrl(input) {
  var str = (input != null && input !== '') ? String(input).trim() : '';
  if (!str) return 'Visited page';
  try {
    var u = new URL(str.startsWith('http') ? str : 'https://' + str);
    return u.hostname.replace(/^www\./, '') || str;
  } catch (_) {
...
function renderHistoryList(items) {
  ...
  var typeLabels = { fake: 'Fake site check', compare: 'App compare', legal: 'Legal risk check' };
  var order = ['fake', 'compare', 'legal'];
  var byType = { fake: [], compare: [], legal: [] };
...
function openSettingsPanel() {
  ...
  refreshHistoryUI();
  setTimeout(refreshHistoryUI, 250);
  syncThemeRadios();
}
