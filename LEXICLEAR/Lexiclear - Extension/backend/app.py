...
def detect_fake_advanced():
    ...
    Multi-layered detection using real APIs and analysis.
    ...
    try:
        ...
        # Validate URL
        validated_url, error = _validate_url(url)
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            ...
            f_safe_browsing = executor.submit(_check_google_safe_browsing, url)
            
            # Layer 4: VirusTotal
            f_virustotal = executor.submit(_check_virustotal, url)
            
            # Layer 5: URL Structure
            f_url_struct = executor.submit(_analyze_url_structure, url)
            
            # Wait for results
            age_days, age_success = f_age.result(timeout=WHOIS_TIMEOUT + 2)
            ssl_info = f_ssl.result(timeout=5)
            safe_browsing = f_safe_browsing.result(timeout=API_TIMEOUT + 2)
            virustotal = f_virustotal.result(timeout=API_TIMEOUT + 2)
            url_structure = f_url_struct.result(timeout=2)
        ...
        signals["url_structure_score"] = url_structure.get("score", 0)
        signals["ssl_info"] = ssl_info
        signals["url_scheme"] = parsed.scheme
        
        # Fetch page content for content analysis
        ...
        # Check legal pages
        missing_pages = int(_check_missing_legal_pages(url) / FAKE_WEIGHTS["missing_legal_per_page"])
        signals["missing_legal_pages"] = missing_pages
        
        # DNS check (optional, non-blocking)
        try:
            dns_info = _check_dns_records(domain)
            signals["dns_info"] = dns_info
        except:
            pass
        ...
        # Safety score: 100 = safest, 0 = riskiest (inverse of risk_score)
        safety_score = 100 - risk_score
        
        # List which checks were actually used (so user knows what "Safe" is based on)
        checks_used = [
            "Domain age (WHOIS)",
            "SSL certificate",
            "URL structure",
